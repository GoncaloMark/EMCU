...
void idle_run(void)
{
    ...
    adc_run();
    adcsim_run(t.vout, out.vout, b.voltage);
    // do nothing
...
int main(void) {
    ...
    while(1){
        ...
        // battery is currently connected directly to turbine (until vout is controlled, remove this line when vout is generated by this control software)
        // 0.7V drop in the output diode is the only difference between Vin and Vout
        if (in.vin <= 70)
            out.vout = 0;
        else
            out.vout = in.vin - 70;
